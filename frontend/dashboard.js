...
async function deleteDocument(button){const id=button.dataset.deleteDoc,name=button.dataset.name||"该文档";
  const message=`确定永久删除《${name}》吗？\n\n将同时删除：\n- MinIO 原文件和提取图片\n- Qdrant Chunk 向量（如存在）\n- PostgreSQL 文档、Chunk 和标签关联记录\n\n删除后不可恢复。`;
  if(!window.confirm(message))return;button.disabled=true;try{await Api.request("/api/knowledge/documents/delete",{method:"POST",body:JSON.stringify({document_id:id})});await pages[button.dataset.refresh]();}catch(e){window.alert(e.message);button.disabled=false;}}
async function reprocessDocument(button){const id=button.dataset.reprocessDoc,name=button.dataset.name||"该文档";
  if(!window.confirm(`确定重新处理《${name}》吗？\n\n向量阶段失败会复用已有 Chunk；更早阶段失败会读取 MinIO 原文件重新处理。`))return;
  button.disabled=true;try{await Api.request("/api/knowledge/documents/reprocess",{method:"POST",body:JSON.stringify({document_id:id})});await pages[button.dataset.refresh]();}catch(e){window.alert(e.message);button.disabled=false;}}
function retryButton(x,refresh){return x.status==="FAILED"?`<button class="retry-button" data-reprocess-doc="${x.id}" data-name="${esc(x.name)}" data-refresh="${refresh}">重新处理</button>`:"";}
function bindDocs(){document.querySelectorAll("[data-doc]").forEach(n=>n.onclick=()=>detail(n.dataset.doc));document.querySelectorAll("[data-delete-doc]").forEach(n=>n.onclick=e=>{e.stopPropagation();deleteDocument(n);});document.querySelectorAll("[data-reprocess-doc]").forEach(n=>n.onclick=e=>{e.stopPropagation();reprocessDocument(n);});}
const pages={};
pages.dashboard=async()=>{const d=await Api.request("/api/dashboard/stats"),c=d.counts;
  ...
  <div class="two-column"><section class="panel"><h2>最近上传知识</h2>${d.recent.map(x=>`<div class="list-row"><button class="document-link" data-doc="${x.id}"><span>${esc(x.name)}</span>${badge(x.status)}</button>${retryButton(x,"dashboard")}<button class="delete-button" data-delete-doc="${x.id}" data-name="${esc(x.name)}" data-refresh="dashboard">删除</button></div>`).join("")||"暂无数据"}</section>
  <section class="panel"><h2>最近失败文档</h2>${d.failed.map(x=>`<div class="list-row"><button class="document-link" data-doc="${x.id}"><span>${esc(x.name)}</span><small>${esc((x.metadata||{}).error_stage||"FAILED")}</small></button>${retryButton(x,"dashboard")}<button class="delete-button" data-delete-doc="${x.id}" data-name="${esc(x.name)}" data-refresh="dashboard">删除</button></div>`).join("")||"暂无失败"}</section></div>`;bindDocs();};
pages.upload=async()=>{const [strategies,systemConfig]=await Promise.all([Api.request("/api/strategies/list"),Api.request("/api/system/config")]),deepseek=systemConfig.deepseek||{},textModels=deepseek.text_models||[deepseek.model],visionModel=deepseek.vision_model;content.innerHTML=`<section class="panel form-panel"><h2>上传投资资料</h2>
  <p>支持 DOCX、PDF、TXT、JPEG、PNG、GIF、WebP；扫描型 PDF 暂不支持 OCR。</p><form id="upload" class="form-grid">
  <label class="file-drop full">知识文件<input id="knowledge-file" name="file" type="file" accept=".docx,.pdf,.txt,.jpg,.jpeg,.png,.gif,.webp" required></label>
  ...
  <label>分析模型<select id="analysis-model" name="analysis_model"></select></label>
  <button class="full">上传并智能入库</button></form><div id="progress"></div></section>`;
  const fileInput=document.querySelector("#knowledge-file"),modelSelect=document.querySelector("#analysis-model"),imageExts=new Set(["jpg","jpeg","png","gif","webp"]);
  const updateModels=()=>{const ext=(fileInput.files[0]?.name.split(".").pop()||"").toLowerCase(),models=imageExts.has(ext)?[visionModel]:textModels;modelSelect.innerHTML=models.filter(Boolean).map(x=>`<option value="${esc(x)}" ${x===deepseek.model?"selected":""}>${esc(x)}</option>`).join("");};
  fileInput.onchange=updateModels;updateModels();
  ...
  try{const doc=await Api.request("/api/knowledge/documents/upload",{method:"POST",body:new FormData(e.currentTarget)});
  for(;;){const s=await Api.request(`/api/knowledge/documents/status/${doc.id}`),m=s.metadata||{};p.innerHTML=`<div class="progress-card ${s.status==="FAILED"?"failed":""}">
...
pages.documents=async()=>{const d=await Api.request("/api/knowledge/documents/list");content.innerHTML=`<section class="panel"><div class="panel-title"><h2>知识管理</h2><button id="new">上传知识</button></div>
  <div class="table-wrap"><table><thead><tr><th>文件名称</th><th>来源</th><th>分类</th><th>策略</th><th>Chunk</th><th>已审核</th><th>状态</th><th>操作</th></tr></thead><tbody>
  ${d.map(x=>`<tr data-doc="${x.id}"><td>${esc(x.name)}</td><td>${esc(x.source_name||x.source_type||"-")}</td><td>${esc(x.category||"-")}</td>
  <td>${esc(x.strategy_name||"未关联")}</td><td>${x.chunk_count}</td><td>${x.reviewed_count}/${x.chunk_count}</td><td>${badge(x.status)}</td><td><div class="actions">${retryButton(x,"documents")}<button class="delete-button" data-delete-doc="${x.id}" data-name="${esc(x.name)}" data-refresh="documents">删除</button></div></td></tr>`).join("")}</tbody></table></div></section>`;
...
async function detail(id){const [d,strategies]=await Promise.all([Api.request(`/api/knowledge/documents/detail/${id}`),Api.request("/api/strategies/list")]);heading.textContent="知识详情";
  ...
  document.querySelector("#raw").onclick=async()=>{const x=await Api.request(`/api/knowledge/documents/raw-url/${id}`);window.open(x.url,"_blank","noopener");};
  document.querySelector("#parsed").onclick=()=>document.querySelectorAll("details.chunk").forEach(x=>x.open=true);bindChunkEditors(id);}
function bindChunkEditors(documentId){document.querySelectorAll("[data-chunk]").forEach(card=>{const chunkId=card.dataset.chunk,msg=card.querySelector(".operation-message"),run=async(path,payload={})=>{msg.textContent="处理中…";try{await Api.request(path,{method:"POST",body:JSON.stringify({chunk_id:chunkId,...payload})});await detail(documentId);}catch(e){msg.textContent=e.message;}};
  card.querySelector(".content-context-form").onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget);run("/api/knowledge/chunks/update-content-context",{content:f.get("content"),context:f.get("context")});};
  card.querySelector(".summary-form").onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget);run("/api/knowledge/chunks/update-summary",{summary:f.get("summary")});};
  card.querySelector(".metadata-form").onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget),payload={title:f.get("title"),chunk_type:f.get("chunk_type"),strategy_id:f.get("strategy_id")||null,tags:String(f.get("tags")||"").split(/[,，]/).map(x=>x.trim()).filter(Boolean)};run("/api/knowledge/chunks/update-metadata",payload);};
  card.querySelector('[data-action="regenerate"]').onclick=()=>run("/api/knowledge/chunks/regenerate-context");card.querySelector('[data-action="review"]').onclick=()=>run("/api/knowledge/chunks/mark-reviewed");
  card.querySelector('[data-action="retrieval"]').onclick=()=>{const enabling=card.querySelector('[data-action="retrieval"]').textContent.includes("恢复");run(`/api/knowledge/chunks/${enabling?"enable":"disable"}-retrieval`);};card.querySelector('[data-action="reindex"]').onclick=()=>run("/api/knowledge/chunks/reindex");});}
...
async function strategyPage(core){const d=await Api.request("/api/strategies/list"),s=core?d.filter(x=>x.category==="core_strategy"):d;content.innerHTML=`<div class="strategy-grid">${s.map(x=>`<article class="strategy-card"><span>${esc(x.category)}</span><h2>${esc(x.name)}</h2>
...
pages.tags=async()=>{const d=await Api.request("/api/tags/list");content.innerHTML=`<section class="panel"><h2>标签管理</h2><div class="tag-cloud">${d.map(x=>`<span><strong>${esc(x.name)}</strong><small>${x.chunk_count}</small></span>`).join("")||"暂无标签"}</div></section>`;};
pages.settings=async()=>{const d=await Api.request("/api/system/config"),sets=[["DeepSeek",d.deepseek],["Embedding",d.embedding],["MinIO",d.minio],["Qdrant",d.qdrant]];
  ...
  document.querySelectorAll(".tester").forEach(b=>b.onclick=async()=>{const o=b.closest(".panel").querySelector(".test-result");o.textContent="测试中…";try{const x=await Api.request("/api/system/services/test",{method:"POST",body:JSON.stringify({service_name:b.dataset.test})});o.textContent=x.success?`连接正常${x.dense_dimension?"，Dense "+x.dense_dimension+"维，Sparse "+x.sparse_terms+"项":""}`:"连接异常";}catch(x){o.textContent=x.message;}});};
...
(async()=>{try{await Api.request("/api/auth/check-login");document.querySelector("#username").textContent=localStorage.getItem(Api.usernameKey)||"用户";menu();await openPage("总览 Dashboard");}
